...
│   │   ├── spec-and-jira-required.md  # When a spec + Jira backlog are mandatory (solo sessions included)
...
│   │   ├── jira_mirror.py                  # Config loading + local mirror journal (shared by the 4 Jira hooks)
...
│   │   └── spec_gate.py                    # UserPromptSubmit + PreToolUse Write — spec/Jira pre-flight and backstop
...
Seven Python scripts in `.claude/hooks/` machine-enforce the protocol (eight hook entries in `.claude/settings.json`, since `spec_gate.py` is wired twice). Four gate the Atlassian MCP's Jira tools once work is on the board; `claim_gate.py` gates `Write`/`Edit` so a file declared by an unclaimed issue cannot be edited before the issue is claimed; `jira_transition_sentinel_finalize.py` completes the two-phase sentinel consume so a failed transition cannot destroy an attestation; and `spec_gate.py` guards the step before all of it — deciding a spec and a backlog are needed at all. They are wired in `.claude/settings.json` and resolved at runtime via `$CLAUDE_PROJECT_DIR/.claude/hooks/...` (this repo) or `$HOME/.claude/hooks/...` (when installed globally — see "Hook path resolution" in [Quick Start](#quick-start)). Every decision is audited to `~/.claude/logs/team-hooks.jsonl`, and all hooks are **fail-open**: any unexpected condition — including no `.claude/jira-config.json` yet — allows the action, so a hook bug (or a not-yet-bootstrapped project) can never block a teammate.
...
| `UserPromptSubmit` | — | `spec_gate.py prompt` | Never blocks — when a prompt reads as build work, injects the spec/Jira pre-flight checklist so the decision is made before the first file is written |
| `PreToolUse` | `Write` | `spec_gate.py write` | Blocks **once** per session when a third new project source file is created and no `.claude/specs/*/spec.md` was written that session; writing a spec clears it, `.claude/**` and docs never count, and `CLAUDE_SPEC_GATE=off` bypasses it |
